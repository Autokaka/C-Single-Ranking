...
void LibraryCard::printID(int duzhebianhao)
{
	cout << "\n__________________________________________________用户信息__________________________________________________\n" << endl;
	cout << left << setw(20) << "读者编号" << setw(20) << "姓名" << setw(20) << "密码" << setw(20) << "借阅等级" << setw(20) << "借书数限制" << setw(20) << "借阅图书条码号" << endl;
	string Client;
	fstream inout("ClientID.txt");
	if (!inout.is_open())
	{
	...
	while (!inout.eof())
	{
		...
		if (Number == duzhebianhao)
		{
			cout << left << setw(20) << Number << setw(20) << Name << setw(20) << Password << setw(20) << Level << setw(20) << Limit << setw(100) << Bookcode << endl;
			return;
		}
	}
}
